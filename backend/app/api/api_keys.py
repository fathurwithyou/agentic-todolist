...
Uses domain services for LLM API key operations.
"""
...
# Backward compatibility endpoints with user_id in path
@router.get("/list/{user_id}", response_model=APIKeysListResponse)
async def list_api_keys_with_user_id(
    user_id: str, current_user=Depends(get_current_user)
):
    """List which providers have API keys saved (backward compatibility)"""
...
@router.get("/test/{user_id}/{provider}")
async def test_api_key_with_user_id(
    user_id: str, provider: str, current_user=Depends(get_current_user)
):
    """Test if an API key is valid (backward compatibility)"""
    # Ensure user can only test their own keys
    if user_id != current_user.user.user_id:
        raise HTTPException(status_code=403, detail="Access denied")
